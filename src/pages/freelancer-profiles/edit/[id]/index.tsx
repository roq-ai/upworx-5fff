...
import { useRoqClient, useFreelancerProfileFindFirst } from 'lib/roq';
...
function FreelancerProfileEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFreelancerProfileFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FreelancerProfileInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.freelancer_profile.update({
        data: values as RoqTypes.freelancer_profile,
        where: {
          id,
        },
      });
